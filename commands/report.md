...
```bash
gh issue list --state open --limit 20 --json number,title,labels,assignees,createdAt | jq '.[] | {number, title, labels: [.labels[].name], age: .createdAt}'
```
...
```bash
gh pr list --state open --json number,title,reviewDecision,isDraft,statusCheckRollup | jq '.[] | {number, title, reviewDecision, isDraft, ci: (.statusCheckRollup | map(.conclusion) | unique)}'
```
